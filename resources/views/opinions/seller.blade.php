            ...
            <span class="font-semibold">Opinie do produktu: </span> {{$product->name}}
             @foreach ($product->opinions as $op)
                <x-bladewind::card>
                    ...
                    <br>
                    <form action="{{route('opinion.report', $op)}}" method="post">
                        @csrf
                        <x-bladewind::button can_submit='true' icon='information-circle' icon_right="true" color='red'>Zgłoś opinię</x-bladewind::button>
                    </form>
                </x-bladewind::card>
            @endforeach
        </x-bladewind::card>
