<x-app-layout>
    <div class="flex flex-col-reverse pt-4 lg:flex-row">
                            ...
                            <form action="{{route('purchase.changeDeliveryMethod')}}" method="post" class='flex justify-between'>
                                ...
                                <div class="flex flex-col">
                                    @foreach ($deliveryTypes as $deliveryType)
                                        @if ($deliveryType->id == $products[0]['delivery']->id)
        ...
        <x-bladewind::centered-content size="tiny">
            <x-bladewind::card class="hover:shadow-gray-300">
                <div class='flex justify-end'>
                    <span class="font-semibold mr-1">Kwota zakupu: </span> {{$priceWithDelivery}}zł ({{$totalPrice}} + dostawa: {{$deliveryPrice}})
                </div>
            </x-bladewind::card>
            <x-bladewind::card class="hover:shadow-gray-300">
                <div class='flex justify-end'>
                    <form action="{{route('purchase.store')}}" method="post">
                        @csrf
                        <x-bladewind::button can_submit='true' icon='wallet' icon_right="true" color='green'>Zamów i zapłać</x-bladewind::button>
                    </form>
                </div>
            </x-bladewind::card>
        </x-bladewind::centered-content>
